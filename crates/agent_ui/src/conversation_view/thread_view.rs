    ...
    fn render_entry(
        ...
        let primary = match &entry {
            ...
            AgentThreadEntry::AssistantMessage(AssistantMessage {
                ...
                style.paragraph_line_height = rems(1.45).into();
...
mod tests {
    ...
    fn captured_tool_call(
        ...
        tool_call.raw_input = update.get("rawInput").cloned();
        tool_call.raw_output = update.get("rawOutput").cloned();
        let mut tool_call = acp_thread::ToolCall::from_acp_for_test(tool_call, status, cx)
            .expect("the capture converts to a tool call");
        // Narration reads the label, which must be the capture's raw title
        // rather than one derived from the tool kind.
        tool_call.label = cx.new(|cx| markdown::Markdown::new(title.into(), None, None, cx));
        tool_call
    }
